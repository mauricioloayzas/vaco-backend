...
const SUGARCANE_STATUS_TRIGGER_FIELDS = [
    'sorbate_grams_used', 'benzoate_grams_used', 'metabisulfite_grams_used',
    'bentonite_grams_used', 'albumin_grams_used',
];

const SUGARCANE_RECALC_TRIGGER_FIELDS = [
    'juice_liters', 'sugarcane_brix', 'final_brix_desired',
...
return function (array $event) {
    ...
    try {
        ...
        $existing = $repository->getBatchSugarCaneWineDetail($id);
        if ($existing === null) {
            return [
                'statusCode' => 404,
        ...
        if (!empty(array_intersect(array_keys($body), SUGARCANE_RECALC_TRIGGER_FIELDS))) {
            ...
            $sugarcaneBrix    = (float)($body['sugarcane_brix'] ?? $existing->sugarcane_brix);
        ...
        if (!empty(array_intersect(array_keys($body), SUGARCANE_STATUS_TRIGGER_FIELDS))) {
